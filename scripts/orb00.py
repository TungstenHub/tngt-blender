    ...
    output float Density = 0.5)
{
    ...
    float wx = exp(-r)*cos(-Time);
    float wy = exp(-r)*sin(-Time);
    Hue = atan2(-wy,-wx)/6.28+0.5;
    Density = (wx*wx+wy*wy)*1.5;
}
'''
formula.write(s)
...
if mode == 'image':
    ...
    bpy.data.scenes['Scene'].render.filepath = '/tmp/orb00.jpg'
    bpy.ops.render.render(animation=False,write_still=True)
elif mode == 'video':
    bpy.data.scenes['Scene'].render.image_settings.file_format = 'AVI_JPEG'
    bpy.data.scenes['Scene'].render.filepath = '/tmp/orb00.avi'
