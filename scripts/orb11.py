    ...
    output float Density = 0.5)
{
    ...
    float wx = exp(-r)*(cos(-2*Time)*x/r-sin(-2*Time)*y/r);
    float wy = exp(-r)*(cos(-2*Time)*y/r+sin(-2*Time)*x/r);
...
if mode == 'image':
    ...
    bpy.data.scenes['Scene'].render.filepath = '/tmp/orb11.jpg'
    bpy.ops.render.render(animation=False,write_still=True)
elif mode == 'video':
    bpy.data.scenes['Scene'].render.image_settings.file_format = 'AVI_JPEG'
    bpy.data.scenes['Scene'].render.filepath = '/tmp/orb11.avi'
