...
# add surface
bpy.ops.mesh.primitive_xyz_function_surface(x_eq="(u*(1-u**2/3+v**2)/3)/4",
                                            y_eq="(-v*(1-v**2/3+u**2)/3)/4",
                                            z_eq="(u**2-v**2)/4", 
                                            range_u_min=-4, 
                                            range_u_max=4,
                                            range_u_step=128, 
...
surf_mat.diffuse_color = Color([x/255 for x in (0,131,143)])
...
# add camera
bpy.ops.object.camera_add(location=(25,0,0), rotation=(pi/2,0,pi/2))
...
if mode == 'image':
    ...
    bpy.data.scenes['Scene'].render.filepath = '/tmp/enneper.jpg'
    bpy.ops.render.render(animation=False,write_still=True)
elif mode == 'video':
    bpy.data.scenes['Scene'].render.image_settings.file_format = 'AVI_JPEG'
    bpy.data.scenes['Scene'].render.filepath = '/tmp/enneper.avi'
